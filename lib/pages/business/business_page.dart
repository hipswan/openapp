...
class _BusinessPageState extends State<BusinessPage> {
  ...
  Widget build(BuildContext context) {
    final Widget _calendar = Theme(
        data: ThemeData.light().copyWith(
            colorScheme:
                ThemeData.light().colorScheme.copyWith(secondary: Colors.blue)),
        child: _getAgendaViewCalendar(
            _events, _onViewChanged, _calendarController));
    return SafeArea(
      child: _deviceOrientation == Orientation.landscape
          ? Scrollbar(
              child: ListView(
                children: <Widget>[
                  Container(
  ...
  SfCalendar _getAgendaViewCalendar(
    ...
    return SfCalendar(
      ...
      showNavigationArrow: Platform.isMacOS,
      onViewChanged: onViewChanged,
      dataSource: calendarDataSource,
      monthViewSettings: MonthViewSettings(
          showAgenda: true, numberOfWeeksInView: Platform.isMacOS ? 2 : 6),
